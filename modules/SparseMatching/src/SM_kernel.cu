...
*return:
**********************************************/
__global__ void sparse_matching_forward(const int num_ele, const int channel, const int height, const int width, const int max_disp, 
                                        const float* ref_feas_data, const float* tar_feas_data,
                                        const float* ref_mask_data, const float* tar_mask_data,
...
*return:
**********************************************/
__global__ void sparse_matching_ref_backward(const int num_ele, const int channel, const int height, const int width, const int max_disp, 
                                             const float* ref_feas_data, const float* tar_feas_data,
                                             const float* ref_mask_data, const float* tar_mask_data,
    ...
    for(int disp=0; disp<cur_max_disp; disp++) {
        ...
        tmp_grad += tmp_sim * tar_feas_data[base_3D-disp] * (disp-output_data[base_2D]);
    }
...
__global__ void check_max_cost1(const int num_ele, const int channel, const int height, const int width, const int max_disp, 
                                const float* ref_feas_data, const float* tar_feas_data,
                                const float* ref_mask_data, const float* tar_mask_data,
                                const float* max_cost_data) {
    ...
    for(int disp=0; disp<cur_max_disp; disp++) {
        ...
        if(max_cost < cost)
            printf("check max_cost1: %f   %f - %d %d\n", max_cost, cost, index, id_batch*step + id_height*width + id_width);
    }
}

__global__ void check_max_cost2(const int num_ele, const int channel, const int height, const int width, const int max_disp, 
                                const float* ref_feas_data, const float* tar_feas_data,
                                const float* ref_mask_data, const float* tar_mask_data,
                                const float* output_data,
                                const float* sum_sim_data, const float* max_cost_data,
    ...
    for(int disp=0; disp<cur_max_disp; disp++) {
        ...
        if(max_cost_data[base_2D_shift]<cost)
            printf("check max_cost2 (%d,%d,%d,%d): %f   %f\n", id_batch, id_channel, id_height, id_width, max_cost_data[base_2D_shift], cost);
    }
}
...
*return:
**********************************************/
__global__ void sparse_matching_tar_backward(const int num_ele, const int channel, const int height, const int width, const int max_disp, 
                                             const float* ref_feas_data, const float* tar_feas_data,
                                             const float* ref_mask_data, const float* tar_mask_data,
                                             const float* output_data,
                                             const float* sum_sim_data, const float* max_cost_data,
...
void sparse_matching_kernel_forward (at::Tensor ref_feas, at::Tensor tar_feas,
	...
	sparse_matching_forward <<< blocks, CUDA_NUM_THREADS >>> (num_ele, channel, height, width, max_disp, ref_feas_data, tar_feas_data, ref_mask_data, tar_mask_data, max_cost_data, output_data, sum_sim_data);
}

void sparse_matching_kernel_backward (at::Tensor ref_feas, at::Tensor tar_feas,
                                      at::Tensor ref_mask, at::Tensor tar_mask,
                                      at::Tensor output,
                                      at::Tensor sum_similarities, at::Tensor max_cost,
    ...
    const int num_ele = grad_ref.numel();
	int blocks = (num_ele + CUDA_NUM_THREADS - 1) / CUDA_NUM_THREADS;
	...
	sparse_matching_ref_backward <<< blocks, CUDA_NUM_THREADS >>> (num_ele, channel, height, width, max_disp, ref_feas_data, tar_feas_data, ref_mask_data, tar_mask_data, output_data, sum_sim_data, max_cost_data, grad_output_data, grad_ref_data);
    
	sparse_matching_tar_backward <<< blocks, CUDA_NUM_THREADS >>> (num_ele, channel, height, width, max_disp, ref_feas_data, tar_feas_data, ref_mask_data, tar_mask_data, output_data, sum_sim_data, max_cost_data, grad_output_data, grad_tar_data);
}
